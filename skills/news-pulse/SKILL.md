---
name: news-pulse
description: Scans recent company, industry, and macro news for signal versus noise and investment-thesis impact.
use_when: Use when the user asks for recent news monitoring, thesis-impact review, catalyst scan, or signal/noise filtering.
source_url: https://raw.githubusercontent.com/xbtlin/ai-berkshire/main/skills/news-pulse.md
...
This Agent Skill is converted from `xbtlin/ai-berkshire/skills/news-pulse.md` for cross-agent compatibility. Follow the upstream workflow below, while applying these portability rules:
...
- **description**:
  ...
  4. **大 V 观点**：可调用 `python3 tools/xueqiu_scraper.py` 抓段永平等大 V 最近相关发言
     ...
     - 若环境未安装 Playwright，改用当前 Agent 的 WebSearch/WebFetch 搜索雪球公开页面，并在报告中标注该工具未运行。
