...
description: Reviews a company earnings release or filing with value-investing discipline, data verification, deltas, and thesis-impact judgment.
use_when: Use when the user asks to analyze an earnings release, 10-Q, 10-K, shareholder letter, guidance update, or quarterly/annual results.
source_url: https://raw.githubusercontent.com/xbtlin/ai-berkshire/main/skills/earnings-review.md
...
This Agent Skill is converted from `xbtlin/ai-berkshire/skills/earnings-review.md` for cross-agent compatibility. Follow the upstream workflow below, while applying these portability rules:

- Treat `$ARGUMENTS` as the user's requested company, ticker, industry, portfolio, theme, or source material.
- Use current primary sources when the task depends on recent filings, prices, news, guidance, or market data.
- Keep verified facts, assumptions, and judgments separate.
- Cite sources for externally retrieved facts.
- Do not present outputs as financial advice; frame conclusions as research judgments.
- If the upstream prompt references Claude-specific tools or commands, use the equivalent tools available in the current agent environment.
